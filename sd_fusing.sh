...
#<BL1 fusing>
echo "BL1 fusing"
sudo dd if=./bl1.bin.hardkernel of=$device seek=$signed_bl1_position
sync

#<BL2 fusing>
echo "BL2 fusing"
sudo dd if=./bl2.bin.hardkernel.720k_uboot of=$device seek=$bl2_position
sync

#<u-boot fusing>
echo "u-boot fusing"
sudo dd if=$uboot of=$device seek=$uboot_position
sync

#<TrustZone S/W fusing>
echo "TrustZone S/W fusing"
sudo dd if=./tzsw.bin.hardkernel of=$device seek=$tzsw_position
sync

#<u-boot env erase>
echo "u-boot env erase..."
sudo dd if=/dev/zero of=$device seek=$env_position bs=512 count=32
sync
